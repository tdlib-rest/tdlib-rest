...
namespace net
{
	...
	int GetMTUWindowsIpv4(sockaddr_in inputAddress, int fallback)
	{
		...
		{
			std::cerr << "NetIface: GetMTU(): enclosed GetAdaptersAddresses() call has failed\n";
			FREE(pAddresses);
			return fallback;
		}
		...
		while(pCurrAddresses)
		{
			...
			if(pUnicast == nullptr)
				std::cerr << "NetIface: GetMTU(): not a unicast ipv4 address, this is not supported\n";
		...
		std::cerr << "NetIface: GetMTU(): no usable unicast ipv4 addresses found\n";
		FREE(pAddresses);
		return fallback;
	}
	...
	int GetMTUWindowsIpv6(sockaddr_in6 inputAddress, int fallback)
	{
		...
		while(pCurrAddresses)
		{
			...
			pUnicast = pCurrAddresses->FirstUnicastAddress;
			if(pUnicast == nullptr)
				std::cerr << "NetIface: GetMTU(): not a unicast ipv6 address, this is not supported\n";
		...
		std::cerr << "NetIface: GetMTU(): no usable unicast ipv6 addresses found\n";
		FREE(pAddresses);
		return fallback;
	}
	...
	int GetMTUWindows(const boost::asio::ip::address& localAddress, int fallback)
	{
		...
		else if(localAddress.is_v6())
		{
			...
			std::cerr << "NetIface: GetMTU(): address family is not supported\n";
			return fallback;
		}
	}
#else // assume unix
	int GetMTUUnix(const boost::asio::ip::address& localAddress, int fallback)
	{
		...
		{
			std::cerr << "NetIface: Can't call getifaddrs(): " << strerror(errno) << "\n";
			return fallback;
		}
		...
		{ // interface found?
			...
			if(fd > 0)
			{
				...
				if(ioctl(fd, SIOCGIFMTU, &ifr) >= 0)
					...
					std::cerr << "NetIface: Failed to run ioctl: " << strerror(errno) << "\n";
				close(fd);
			}
			else
				std::cerr << "NetIface: Failed to create datagram socket\n";
		}
		else
			std::cerr << "NetIface: interface for local address" << localAddress.to_string() << " not found\n";
		freeifaddrs(ifaddr);
	...
	{
#ifdef WIN32
		std::cerr << "NetIface: cannot get address by interface name, not implemented on WIN32\n";
		if(ipv6)
		...
		{
			fallback = "::1";
			std::cerr << "NetIface: cannot find ipv6 address for interface " << ifname << "\n";
		} else {
			fallback = "127.0.0.1";
			std::cerr << "NetIface: cannot find ipv4 address for interface " << ifname << "\n";
		}
